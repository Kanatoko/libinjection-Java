...
*/
    ...
    public static void testParseSingleQuoteString()
        ...
        if (parseSingleQuoteString("'A''B'") != 6) {
            ex();
        }
        if (parseSingleQuoteString("''") != 2) {
            ex();
        }
        if (parseSingleQuoteString("'AA'") != 4) {
            ex();
        }
        if (parseSingleQuoteString("'") != 1) {
            ex();
        }
        if (parseSingleQuoteString("'A") != 2) {
            ex();
        }
    }
    ...
    public static void testParseComment()
        ...
        if (parseCStyleComment("/*bar*/baz", false, null) != 7) {
            ex();
        }
...
//testTokenizeWithWhite( "あ", "n" );
    ...
    public static void testTokenizeOne(final String input, final String expectedProcessed, final String expectedToken)
        ...
        if (!processed[0].equals(expectedProcessed)) {
            ex();
        }
        if (!tokenBuf[0].equals(expectedToken)) {
            ex();
        }
    }
